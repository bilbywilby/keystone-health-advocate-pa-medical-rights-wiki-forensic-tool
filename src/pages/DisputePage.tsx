...
const QUICK_DISPUTES = [
  {
    ...
    cites: 'Federal No Surprises Act',
        ...
        <div className="py-8 md:py-10 lg:py-12 space-y-12">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {QUICK_DISPUTES.map((dispute, i) => (
              <Card key={i} className="hover:shadow-lg transition-all group border-slate-200 overflow-hidden">
                <CardHeader>
                  <div className="flex items-center justify-between mb-2">
                    <div className="p-2 bg-indigo-50 text-indigo-600 rounded-lg dark:bg-indigo-900/30">
                ...
                <CardContent>
                  <Button asChild className="w-full bg-slate-900 hover:bg-amber-500 transition-colors text-white">
                    <Link to={`/appeal-generator?type=${dispute.type}`}>
                      Generate Dispute Letter <ArrowRight className="w-4 h-4 ml-2" />
                    </Link>
                  </Button>
                </CardContent>
              </Card>
